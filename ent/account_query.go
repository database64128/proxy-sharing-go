...
func (aq *AccountQuery) QueryServers() *ServerQuery {
	query := (&ServerClient{config: aq.config}).Query()
...
func (aq *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	...
	if query := aq.withServers; query != nil {
		if err := aq.loadServers(ctx, query, nodes,
			func(n *Account) { n.Edges.Servers = []*Server{} },
			func(n *Account, e *Server) { n.Edges.Servers = append(n.Edges.Servers, e) }); err != nil {
			return nil, err
		}
	}
...
func (aq *AccountQuery) loadServers(ctx context.Context, query *ServerQuery, nodes []*Account, init func(*Account), assign func(*Account, *Server)) error {
	...
	query.Where(predicate.Server(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(account.ServersColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.account_servers
		if fk == nil {
			return fmt.Errorf(`foreign-key "account_servers" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "account_servers" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (aq *AccountQuery) loadNodes(ctx context.Context, query *NodeQuery, nodes []*Account, init func(*Account), assign func(*Account, *Node)) error {
	...
	for _, n := range neighbors {
		fk := n.account_nodes
		if fk == nil {
			return fmt.Errorf(`foreign-key "account_nodes" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
