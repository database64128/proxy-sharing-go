...
func (nq *NodeQuery) QueryServer() *ServerQuery {
	query := (&ServerClient{config: nq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := nq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (nq *NodeQuery) Clone() *NodeQuery {
	...
	return &NodeQuery{
		...
		withServer:  nq.withServer.Clone(),
...
func (nq *NodeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Node, error) {
	...
	if nq.withAccount != nil || nq.withServer != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := nq.withAccount; query != nil {
		if err := nq.loadAccount(ctx, query, nodes, nil,
			func(n *Node, e *Account) { n.Edges.Account = e }); err != nil {
			return nil, err
		}
	}
	if query := nq.withServer; query != nil {
		if err := nq.loadServer(ctx, query, nodes, nil,
			func(n *Node, e *Server) { n.Edges.Server = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (nq *NodeQuery) loadAccount(ctx context.Context, query *AccountQuery, nodes []*Node, init func(*Node), assign func(*Node, *Account)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Node)
	for i := range nodes {
		if nodes[i].account_nodes == nil {
			continue
		}
		fk := *nodes[i].account_nodes
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "account_nodes" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (nq *NodeQuery) loadServer(ctx context.Context, query *ServerQuery, nodes []*Node, init func(*Node), assign func(*Node, *Server)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Node)
	for i := range nodes {
		if nodes[i].server_nodes == nil {
			continue
		}
		fk := *nodes[i].server_nodes
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "server_nodes" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
